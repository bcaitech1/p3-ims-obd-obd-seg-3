...
import numpy as np
import math
import gscnn_utils.network.mynn as mynn
import gscnn_utils.my_functionals.custom_functional as myF
class GatedSpatialConv2d(_ConvNd):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1,
        ...
        self._gate_conv = nn.Sequential(
            mynn.Norm2d(in_channels+1),                         # 여기서 channel +1 된 이유는 gating_features가 concat되서임
            nn.Conv2d(in_channels+1, in_channels+1, 1),
            nn.ReLU(), 
            nn.Conv2d(in_channels+1, 1, 1),                     # shape을 (B, 1, H, W) 로 나타내어줌
            mynn.Norm2d(1),
            nn.Sigmoid()
        )
    ...
    def forward(self, input_features, gating_features):
        ...
        :return:
        """
        alphas = self._gate_conv(torch.cat([input_features, gating_features], dim=1))       # 마지막에 sigmoid니깐 가중치의 형태로 출력됨

        input_features = (input_features * (alphas + 1))                                    # 가충치 +1을 해줬으니 기존 Feature에서 더 커지는 형태가 됨
